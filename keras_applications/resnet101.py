...
def identity_block(input_tensor, kernel_size, filters, stage, block, use_bias=False):
    ...
    x = Convolution2D(nb_filter1, 1, 1, name=conv_name_base + '2a', bias=use_bias)(input_tensor)
    ...
    x = Convolution2D(nb_filter2, kernel_size, kernel_size,
                      name=conv_name_base + '2b', bias=use_bias)(x)
    x = BatchNormalization(epsilon=eps, axis=bn_axis, name=bn_name_base + '2b')(x)
    x = Scale(axis=bn_axis, name=scale_name_base + '2b')(x)
    x = Activation('relu', name=conv_name_base + '2b_relu')(x)

    x = Convolution2D(nb_filter3, 1, 1, name=conv_name_base + '2c', bias=use_bias)(x)
...
def conv_block(input_tensor, kernel_size, filters, stage, block, strides=(2, 2), use_bias=False):
    ...
    x = Convolution2D(nb_filter1, 1, 1, subsample=strides,
                      name=conv_name_base + '2a', bias=use_bias)(input_tensor)
    x = BatchNormalization(epsilon=eps, axis=bn_axis, name=bn_name_base + '2a')(x)
    x = Scale(axis=bn_axis, name=scale_name_base + '2a')(x)
    x = Activation('relu', name=conv_name_base + '2a_relu')(x)
    ...
    shortcut = Convolution2D(nb_filter3, 1, 1, subsample=strides,
                             name=conv_name_base + '1', bias=use_bias)(input_tensor)
...
def resnet101_model(weights_path=None,classes=1000):
    ...
    x = Convolution2D(64, 7, 7, subsample=(2, 2), name='conv1', bias=use_bias)(x)
    ...
    x = identity_block(x, 3, [64, 64, 256], stage=2, block='c',use_bias=use_bias)

    x = conv_block(x, 3, [128, 128, 512], stage=3, block='a',use_bias=use_bias)
    for i in range(1,4):
        x = identity_block(x, 3, [128, 128, 512], stage=3, block='b'+str(i),use_bias=use_bias)

    x = conv_block(x, 3, [256, 256, 1024], stage=4, block='a',use_bias=use_bias)
    for i in range(1,23):
        x = identity_block(x, 3, [256, 256, 1024], stage=4, block='b'+str(i),use_bias=use_bias)
    ...
    x = identity_block(x, 3, [512, 512, 2048], stage=5, block='c',use_bias=use_bias)
